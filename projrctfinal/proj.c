...
void initiateNewElection()
{
    ...
    printf("%s", currentValidID.branch);
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            int WinnerCid, totalVotedNow = 0;
             red();
            printf("\n************________WELCOME TO ADMIN PANEL________************");
            reset();
            green();
...
{system("cls");
    system("color 40");
    red();
    printf("***********>>>>>>____WELCOME TO ELECTIONAL PORTAL____<<<<<<<<************");
    reset();
    while (1)
    {green();
        printf("\n\t\t\t   1.>>Student panel \n\t\t\t   2.>>Admin panel \n\t\t\t   3.>>Exit \n\t\t\t   Option:");
